...
- **Memory graph** (`graph.py`, July 2026): derived nodes + edges over items.db (same sqlite file). Raw captures stay untouched; derived nodes (summary/pattern/observation/symbol/question/tension) cite sources via `derived_from` edges. FTS5 recall with one-hop expansion, character budgets, a retrieval log, and touch counts. Node/edge statuses carry review state: `open`/`accepted` participate in recall, `proposed` waits for human review, `rejected` is remembered and final, `held` is the partner's private layer (out of recall, readable by the dreamer).
- **Autodreamer** (`dreamer/`, July 2026): the nightly consolidation pass, ported from the silas_dream prototype against the graph. Stages: verbalized residue (the partner's own voice, primary tier — "what are you still holding that you never said?"; quiet days skip; mind-reading lines dropped at storage) → co-recall associations (deterministic mining of the retrieval log; lift-or-confidence qualification; let-go pairs never re-proposed) → collect (stratified selection over items/nodes/conversation/letters/residue) → metabolize (injected `complete()` callable, cheap tier) → validate (the deterministic boundary layer: support alignment, computed recurrence, no mind-reading, ontology constraints, caps) → apply (graph proposal states) → summarize. Write boundary enforced by `_safe_write`. The dreamer holds no keys; callers inject model callables.
